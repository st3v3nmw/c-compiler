...
string scanNumber() {
  ...
  infile >> noskipws >> ch;
  while (isdigit(ch) &&
         !infile.eof()) { // scan number before decimal point (if any exists)
    buffer += ch;
    infile >> noskipws >> ch;
  }
  ...
  while (infile >> noskipws >> ch) {
    ...
    case '/':
      ...
      if (ch == '/') { // match comment
        ...
        token_end += 1;
        tokens.push_back(
            TokenNode(T_SLASH_ASSIGN, "/=", line_number, token_end));
      } else {
    ...
    case '*':
      ...
      if (ch == '=') {
        token_end += 1;
        tokens.push_back(
            TokenNode(T_STAR_ASSIGN, "*=", line_number, token_end));
      } else {
    ...
    default:
      if (isalpha(ch) ||
          ch == '_') { // match identifier or keyword or boolean literal
        buffer = ch;
        infile >> noskipws >> ch;
        token_end += 1;
        ...
        int tokenType =
            (buffer == "true" || buffer == "false") ? T_BOOL_LIT : T_IDENTIFIER;
        ...
        if (point == string::npos) // integer
          tokens.push_back(
              TokenNode(T_INT_LIT, buffer, line_number, token_end));
        else // float
          tokens.push_back(
              TokenNode(T_FLOAT_LIT, buffer, line_number, token_end));
      } else if (ch == '+' || ch == '-') {
        if (isdigit(infile.peek())) { // match signed number (float or int)
          ...
          if ((last.token >= T_PLUS && last.token <= T_SLASH) ||
              (last.token >= T_ASSIGN && last.token <= T_SLASH_ASSIGN)) {
            if (point == string::npos) // integer
              tokens.push_back(
                  TokenNode(T_INT_LIT, ch + buffer, line_number, token_end));
            else // float
              tokens.push_back(
                  TokenNode(T_FLOAT_LIT, ch + buffer, line_number, token_end));
          } else {
            tokens.push_back(TokenNode(ch == '+' ? T_PLUS : T_MINUS,
                                       ch == '+' ? "+" : "-", line_number,
                                       token_end));
        ...
        } else if (infile.peek() == ch &&
                   (infile.peek() == '+' |
                    infile.peek() == '-')) { // match unary ++ or --
          tokens.push_back(TokenNode(
              infile.peek() == '+' ? T_UNARY_PLUS : T_UNARY_MINUS,
              infile.peek() == '+' ? "++" : "--", line_number, token_end));
          infile >> noskipws >> ch; // consume last + or - and discard it
          token_end += 1;
        } else {
          if (infile.peek() == '=') {
            tokens.push_back(
                TokenNode(ch == '+' ? T_PLUS_ASSIGN : T_MINUS_ASSIGN,
                          ch == '+' ? "+=" : "-=", line_number, token_end));
        ...
        if (ch == '\n') {
          ...
          token_end = 0;
                     ...
                     ch == '\r')) // ignore whitespace
          cout << "Error [line " << line_number + 1
